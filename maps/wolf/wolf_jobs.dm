...
/datum/job/wolf/command/captain/get_description_blurb()
	var/desc = {"You are the Captain assigned to the FTE Odyssey. In your hands is a significant investment of trust from the Company - and the lives of everyone on your ship.
	Lead them well, complete the mission, and come home alive. Your mission is to patrol the border systems between the Gimli Republic and deep space. Along the way, based on findings,
	you are to collect salavge to repair your own ship, collect samples of xenoflora and fauna, and engage hostile ships. Your ship is highly capable - but only as much as the person leading it."}
	return desc
...
/datum/job/wolf/command/first_officer/get_description_blurb()
	var/desc = {"You are the First Officer aboard the FTE Odyssey. You are the Captain's finger on the pulse of the crew, and the one who executes his orders by passing information down to
...
/datum/job/wolf/security/SSC/get_description_blurb()
	var/desc = {"You are the Ship Security Commander aboard the FTE Odyssey. Chosen for your ability to lead or your skills, you've been placed in charge of the ship's security. A serious task
	considering where it's main area of operations is. Under your purview are a handful of security staff, and the power to deputize anyone who you see fit. Your underlings fly the ship's gunship
	, man it's weapons, and keep the crew in good order. When in combat, you may be right by their side repelling boarders - and watching them die. Give your orders carefully, for life is precious on the frontier."}
	return desc
...
/datum/job/wolf/engineering/CE/get_description_blurb()
	var/desc = {"You are the Chief Engineer aboard the FTE Odyssey. You are a highly skilled professional at least somewhat understanding of how to coordinate a team of enginers to maintain,
...
/datum/job/wolf/engineering/engineer/get_description_blurb()
	var/desc = {"You are an Engineer aboard the FTE Odyssey. A rough and tumble greasemonkey, or a nerd more comfortable with computer systems than fighting, it doesn't matter. You've been hired
...
/datum/job/wolf/medical/CMO/get_description_blurb()
	var/desc = {"You are the Chief Medical Officer aboard the FTE Odyssey. Your job is to keep the crew alive and reasonably healthy within your abilities. You oversee the medical department and
...
/datum/job/wolf/science/TD/get_description_blurb()
	var/desc = {"You are the Technology Director aboard the FTE Odyssey. Almost a token position, all things considered, given the fact that it is a warship. Regardless, you do have a role to
...
/datum/job/wolf/science/research_tech/get_description_blurb()
	...
	to either find whatever you can while the ship patrols, or hired by FENRIS to assist the Technology Director in their mission of making sure that the ship's machinery and such are functioning
...
/datum/job/wolf/civilian/bosun/get_description_blurb()
	var/desc = {"You are the Bosun aboard the FTE Odyssey. Your job is to oversee all the civilian functions of the ship, and ensure that supplies are evenly, and quickly distributed.
...
/datum/job/wolf/civilian/salvage_tech/get_description_blurb()
	var/desc = {"You are a Salvage Technician aboard the FTE Odyssey. FENRIS expects the ship's main source of income to be from valuable materials and technologies recovered from ships the Odyssey
...
/datum/job/assistant/get_description_blurb()
	var/desc = {"You're a Crewmember aboard the FTE Odyssey. Your answer to absolutely everyone. You are a jack of all trades but master of none, answering directly to literally everyone with a preference
